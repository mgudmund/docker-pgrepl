...
if [ -n "$PGREPL_ROLE" ]; then
   ...
   if [ "$PGREPL_ROLE" == "STANDBY" ]; then
      ...
      gosu postgres echo "standby_mode='on'" >> "$PGDATA"/recovery.conf
      gosu postgres echo "primary_conninfo='host=$PGREPL_MASTER_IP port=$PGREPL_MASTER_PORT user=pgrepl'" >> "$PGDATA"/recovery.conf
      gosu postgres echo "recovery_target_timeline = 'latest'" >> "$PGDATA"/recovery.conf
   elif [ "$PGREPL_ROLE" == "PRIMARY" ]; then
